...
function hasCollidedWithSnake() {
  ...
  */
  for(var x = 1; x < snake.body.length; x++){
    if(snake.head.row === snake.body[x].row && snake.head.column === snake.body[x].column){
      return true
    }
    }
  return false;
...
function getRandomAvailablePosition() {
  ...
  while (!spaceIsAvailable) {
    ...
    */
   for(var t = 0; t < snake.body.length; t++){
    if(randomPosition.row === snake.body[t].row && randomPosition.column === snake.body[t].column){
      return false;
    }
   }
  }
